...
def test_project_key_from_cwd_replaces_slashes():
    assert project_key_from_cwd("/home/you/myproject") == "-home-you-myproject"


def test_no_proposals_returns_none(tmp_path: Path):
    journal = tmp_path / "journal"
    (journal / "proposals").mkdir(parents=True)
    out = build_proposal_context(journal_repo=journal, cwd="/home/you/myproject")
    assert out is None
...
def test_lists_only_current_projects_proposals(tmp_path: Path):
    ...
    (proposals / "2026-04-28--home-you-myproject.md").write_text(
    ...
    assert "2026-04-28--home-you-myproject.md" in out
...
def test_uses_first_heading_as_summary(tmp_path: Path):
    ...
    proposals = journal / "proposals"
    proposals.mkdir(parents=True)
    (proposals / "2026-04-29--home-you-myproject.md").write_text(
        "Some preamble\n\n"
...
def test_advertises_journal_slash_command(tmp_path: Path):
    ...
    proposals = journal / "proposals"
    proposals.mkdir(parents=True)
    (proposals / "2026-04-29--home-you-myproject.md").write_text("## something\n")
    out = build_proposal_context(journal_repo=journal, cwd="/home/you/myproject")
...
def test_labels_new_skill_proposal(tmp_path: Path):
    ...
    proposals = journal / "proposals"
    proposals.mkdir(parents=True)
    (proposals / "2026-06-08--home-you-myproject.md").write_text(
